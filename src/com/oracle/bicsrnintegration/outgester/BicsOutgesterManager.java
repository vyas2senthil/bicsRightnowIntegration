...
public class BicsOutgesterManager implements Runnable {
	...
	public BicsOutgesterManager(BicsTableDescription bicsTableDescription) {
		...
		for (int i = 0; i < MAX_WORKERS; i++) workers[i] = new BicsOutgesterWorker(i + 1, bicsTableDescription);
	...
	public void run() {
		...
		while (!done || getQueueSize() > 0) {
			...
			Log.info(Module.BICSOUTGESTERMANAGER, "Found worker...");
